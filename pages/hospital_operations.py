...
with col2:
    doctor_name = st.text_input("Gender", placeholder="M/F")
...
st.markdown(
    ...
    <div class="form-section">
        <div class="section-title">
            <span class="section-icon">📱</span> Billing Details
        </div>
    """,
    unsafe_allow_html=True
)
...
with col1:
    dob = st.date_input("Admit Date", min_value=datetime(1900, 1, 1), max_value=datetime.now())
    dob = st.date_input("Discharge Date", min_value=datetime(1900, 1, 1), max_value=datetime.now())


with col2:
  medicine_cost = st.number_input("Medicine Cost (₹)", min_value=0.0, format="%.2f")
...
allergies = st.text_area("Doctor'Notes", placeholder="Remarks", height=100)
